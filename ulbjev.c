...
void dealerCacheAdd(char * key, float *prob)
{
    ...
    dealerCache.dprob[dealerCache.len] = calloc(11, sizeof(float));
    memcpy(dealerCache.dprob[dealerCache.len], prob, 11 * sizeof(float));
...
void hashDealerPlay(hand_t dh, shoe_t shoe, char *hash)
{
...
void _dealerPlay(shoe_t shoe, hand_t hand, float* prob, float inputProb)
{
    ...
    for (int c = 0; c < 10; ++c) {
        if (shoe.cards[c] > 0
            ...
            int hv = ndhv / 100;
            int sv = ndhv % 100;
            if (ndhv == 1121 && h.length == 2) {
                prob[ 10 ] += inputProb * cp; // hidden blackjack
            } else if (sv >= 17 || hv >= 17) {
...
void dealerPlay(shoe_t shoe, hand_t hand, float* prob, float inputProb) {
    if (inputProb == 1) {
        ...
        if (idx < 0) {
            ...
            memcpy(prob, dealerCache.dprob[idx], sizeof(float) * 11);
        }
        return;
    }
    _dealerPlay(shoe, hand, prob, inputProb);
...
{
    return dealerPlayEV(shoe, playerHand, dealerHand);
}
...
float _dealerPlayEV(shoe_t sh, hand_t ph, hand_t dh)
{
    float ev = 0;
    ...
    ev += -1 * dprob[10]; // dealer hidden/nopeak BJ
    free(dprob);
...
float dealerPlayEV(shoe_t shoe, hand_t playerHand, hand_t dealerHand)
{
    char key[15] = {0};
    float ev = 0;
    hashPlayerActionEV(DEALER, shoe, playerHand, dealerHand, key);
    int idx = playerCacheFind(key);
    if (idx < 0) {
        ev = _dealerPlayEV(shoe, playerHand, dealerHand);
        playerCacheAdd(key, ev);
    } else {
        ev = playerCache.ev[idx];
    }
